...
<template>
    <aside class="w-[30vw] bg-white border-l border-gray-400 flex flex-col overflow-hidden transition-all duration-300"
        ...
        <div class="px-4 py-2 font-bold text-lg border-b border-gray-400 flex items-center justify-between">
            <span>
                {{ $t('cart.title') }}
                <span v-if="cart.cart.items.length" class="text-sm font-normal text-gray-500">
        ...
        <div class="flex-1 overflow-y-auto px-4">
            ...
            <div v-for="(entry, idx) in cart.cart.items" :key="`${entry.type}-${entry.refId}`"
                class="py-2.5 border-b border-gray-300">
                <div class="flex items-center justify-between">
                    <span class="font-semibold">{{ entry.name }}</span>
                    <button @click="cart.removeEntry(idx)"
                        class="text-red-500 hover:text-red-700 text-lg leading-none">×</button>
                </div>
                <div class="flex items-center justify-between mt-2">
                    <div class="flex items-center gap-2">
                        <button @click="cart.updateQty(idx, entry.qty - 1)"
                    ...
                    <span class="font-bold">{{ settings.currencySymbol }}{{ (entry.price * entry.qty).toFixed(2)
                        }}</span>
                </div>
            </div>
        </div>
        ...
        <!-- Dialogs -->
        <ConfirmDialog v-if="showCheckoutConfirm" :title="$t('common.confirm')" :message="$t('cart.confirmCheckout')"
            @confirm="handleCheckout" @cancel="showCheckoutConfirm = false" />
        <ConfirmDialog v-if="showClearConfirm" :title="$t('common.confirm')" :message="$t('cart.confirmClear')"
            @confirm="(cart.clear(), showClearConfirm = false)" @cancel="showClearConfirm = false" />
    </aside>
</template>
